...
class Config {
  static pm2(name) {
    ...
    const filename = path.resolve(`${__dirname}/../config/${name}/pm2.json`);
    try {
  ...
  static app(name) {
    ...
    const filename = path.resolve(`${__dirname}/../config/${name}/app.json`);
    try {
      const config = require(filename);
      if (config.name) {
        Logger.error('Static names for the instances are not supported');
  ...
  static apacheTemplate(name) {
    ...
    const filename = path.resolve(`${__dirname}/../config/${name}/vhost`);
    try {
