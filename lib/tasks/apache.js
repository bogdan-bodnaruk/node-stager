...
class ApacheTask {
  ...
  create() {
    ...
    fs.writeFileSync(path.resolve(fileName), content, (err) => {
      if (err) {
    ...
    const process = exec(`sudo mv ${path.resolve(fileName)} ${dirName}`, { silent: true });
    if (process.stderr) {
  ...
  remove() {
    const dirName = '/etc/apache2/sites-enabled/';
    const fileName = `${this.config.repository}_${this.config.branch}.vhost`;
    Logger.proggress(`Removing apache file: ${fileName}`);

    const progress = exec(`sudo rm -f ${path.join(dirName, fileName)}`, { silent: true });
    if (progress.stderr) {
